...
if [[ "$upload" == "true" ]]; then
  ...
  sshpass -p "$SSH_SERVER_PASSWORD" scp bin/proc.x64.o "${SSH_SERVER_USERNAME}@${SSH_SERVER_IP}:C:/SharedFolder/"
...
if [[ "$interactive" == "true" ]]; then
  ...
  sshpass -p "$SSH_SERVER_PASSWORD" ssh "${SSH_SERVER_USERNAME}@${SSH_SERVER_IP}" "C:/SharedFolder/CoffeeLdr.x64.exe go C:/SharedFolder/proc.x64.o"
fi
